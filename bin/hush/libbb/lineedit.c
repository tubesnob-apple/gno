...
int FAST_FUNC read_line_input(line_input_t *st, const char *prompt, char *command, int maxsize, int timeout)
{
	...
	KTRACE_LOG("rli:init_s");
	INIT_S();

	KTRACE_LOG("rli:ioctl");
#ifndef __GNO__
	...
	KTRACE_LOG("rli:unicode");
	init_unicode();
	KTRACE_LOG("rli:termcap");
	init_termcap();
...
#define command command_must_not_be_used
	...
	KTRACE_LOG("rli:getpwuid");
#if ENABLE_USERNAME_OR_HOMEDIR
	{
		struct passwd *entry;
		/*
		 * ORCA/C 65816 workaround: getpwuid(geteuid()) as a nested
		 * call crashes due to register clobbering during argument
		 * setup.  Split into separate calls with an explicit temp.
		 */
		uid_t myuid = geteuid();
		entry = getpwuid(myuid);
		if (entry) {
