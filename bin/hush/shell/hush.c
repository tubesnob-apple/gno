 ...
 * corrupting them).  Don't use it.
 *
 * Another bug related to pgrp functions:
 * tctpgrp() and settpgrp() don't set a return value on success, so they
 * may return random garbage instead of 0.  Hopefully it can't be -1...
 */
#endif
...
#define debug_printf_parse(x)  do {} while (0)
#define debug_print_tree(a, b)   do {} while (0)
#define debug_printf_exec(x)   do {} while (0)
#define debug_printf_env(x)    do {} while (0)
#define debug_printf_jobs(x)   do {} while (0)
#define debug_printf_expand(x) do {} while (0)
#define debug_printf_varexp(x) do {} while (0)
#define debug_printf_glob(x)   do {} while (0)
#define debug_printf_list(x)   do {} while (0)
#define debug_printf_subst(x)  do {} while (0)
#define debug_printf_clean(x)  do {} while (0)
...
#if !defined(debug_printf) || !defined(debug_printf_parse) || \
	!defined(debug_printf_exec) || !defined(debug_printf_env) || \
	!defined(debug_printf_jobs) || !defined(debug_printf_expand) || \
	!defined(debug_printf_varexp) || !defined(debug_printf_glob) || \
	!defined(debug_printf_list) || !defined(debug_printf_subst) || \
	!defined(debug_printf_clean)
static void real_debug_printf(const char *fmt, ...) {
	va_list p;
...
// Note that x in the below macros must be passed a parenthesized argument list
#ifndef debug_printf
# define debug_printf(x) real_debug_printf x
#endif

#ifndef debug_printf_parse
# define debug_printf_parse(x) real_debug_printf x
#endif

#ifndef debug_printf_exec
#define debug_printf_exec(x) real_debug_printf x
#endif

#ifndef debug_printf_env
# define debug_printf_env(x) real_debug_printf x
#endif

#ifndef debug_printf_jobs
# define debug_printf_jobs(x) real_debug_printf x
# define DEBUG_JOBS 1
#else
# define DEBUG_JOBS 0
#endif
...
# define debug_printf_expand(x) real_debug_printf x
...
# define debug_printf_varexp(x) real_debug_printf x
#endif

#ifndef debug_printf_glob
# define debug_printf_glob(x) real_debug_printf x
# define DEBUG_GLOB 1
#else
# define DEBUG_GLOB 0
#endif
...
# define debug_printf_list(x) real_debug_printf x
#endif

#ifndef debug_printf_subst
# define debug_printf_subst(x) real_debug_printf x
#endif

#ifndef debug_printf_clean
# define debug_printf_clean(x) real_debug_printf x
...
static void die_if_script(const char *fmt, ...)
{
	va_list p;
...
#endif
		KTRACE_LOG("prs:parse");
		pipe_list = parse_stream(NULL, inp, end_trigger);
		KTRACE_LOG("prs:parsed");
 ...
 */
static int save_fds_on_redirect(int fd, int squirrel[])
{
	if (squirrel) {
		/* Handle redirects of fds 0,1,2 */

		/* If we collide with an already moved stdio fd... */
		if (fd == squirrel[STDIN_FILENO]) {
			squirrel[STDIN_FILENO] = xdup_and_close(squirrel[STDIN_FILENO], F_DUPFD);
			return 1;
		}
		if (fd == squirrel[STDOUT_FILENO]) {
			squirrel[STDOUT_FILENO] = xdup_and_close(squirrel[STDOUT_FILENO], F_DUPFD);
			return 1;
		}
		if (fd == squirrel[STDERR_FILENO]) {
			squirrel[STDERR_FILENO] = xdup_and_close(squirrel[STDERR_FILENO], F_DUPFD);
			return 1;
		}
		/* If we are about to redirect stdio fd, and did not yet move it... */
		if (fd <= STDERR_FILENO && squirrel[fd] < 0) {
...
#if ENABLE_HUSH_INTERACTIVE
	if (fd != STDIN_FILENO && fd == G.interactive_fd) {
...
{
	KTRACE_LOG("exec:die");
	if (argv[0]) KTRACE_LOG(argv[0]);
	debug_printf_exec(("execing '%s'\n", argv[0]));
	/* Don't propagate SIG_IGN to the child */
	if (SPECIAL_JOBSTOP_SIGS != 0)
		switch_off_special_sigs(G.special_sig_mask & SPECIAL_JOBSTOP_SIGS);
	signal_parent_to_resume();
	KTRACE_LOG("exec:execvp");
	execvp(argv[0], argv);
	KTRACE_LOGF("exec:fail:errno=%d", errno);
...
# ifndef __GNO__
			...
			if (pi->pgrp < 0)
				pi->pgrp = command->pid;
# else
			if (pi->pgrp != _getpgrp(command->pid))
...
# else
		if (pgrp < 0) {
			...
			int i;
			for (i = 0; i < (*args->pi_p)->num_cmds; i++) {
				pid_t pid = (*args->pi_p)->cmds[i].pid;
				if (pid != 0 && _getpgrp(pid) == pgrp && tctpgrp(G_interactive_fd, pid) != -1)
					goto tty_has_pgrp;
			}
			/* If none of the other processes in the pipe has the right pgrp,
			 * (because they exited or changed their pgrp), allocate a new one. */
			if (getpgrp() != pgrp)
				tcnewpgrp(G_interactive_fd);
		}
 ...
 tty_has_pgrp:
		...
		/* If the terminal shouldn't really be set to the new pgrp 
		 * (e.g. because the pipe is backgrounded), set it back to the parent. */
		if ((*args->pi_p)->followup == PIPE_BG || !G_saved_tty_pgrp)
			tctpgrp(G_interactive_fd, getppid());
# endif
	}
#endif
...
{
	FILE *input;
	KTRACE_LOG("src:fopen");
	input = fopen_for_read(path);
	if (input != NULL) {
		KTRACE_LOG("src:remember");
		remember_FILE(input);
		KTRACE_LOG("src:sighandlers");
		install_special_sighandlers();
		KTRACE_LOG("src:parse");
		parse_and_run_file(input);
		KTRACE_LOG("src:fclose");
		fclose_and_forget(input);
		return 1;
	}
	KTRACE_LOG("src:notfound");
	return 0;
}
...
static void source_user_startup_files(const char * const *files)
{
	...
	KTRACE_LOG("usrc:enter");
	if (getuid() == geteuid() && getgid() == getegid()) {
		KTRACE_LOG("usrc:getHOME");
		home = get_local_var_value("HOME");
		if (home) {
			KTRACE_LOG("usrc:loop");
...
int hush_main(int argc, char **argv)
{
...
#endif
	INIT_G();
	KTRACE_LOG("hush:01:INIT_G done");
	...
	environInit();
	new_cloexec_ent(0);
	KTRACE_LOG("hush:02:GNO init done");
#endif
	...
	KTRACE_LOGF("%s=%d","hush:03:flags", (int)flags);
	builtin_argc = 0;
	while (1) {
...
# endif
...
#endif
	KTRACE_LOG("hush:04:post-option-parse");

	if (G.root_pid == getpid()) {
		KTRACE_LOG("hush:05:sourcing hushenv");
		source_startup_file(global_env);
		source_user_startup_files(user_envs);
		KTRACE_LOG("hush:06:hushenv done");
	}

	/* If we are login shell... */
	if (flags & OPT_login) {
		debug_printf(("sourcing /etc/profile\n"));
		KTRACE_LOG("hush:07:sourcing /etc/profile");
		source_startup_file(global_profile);
		KTRACE_LOG("hush:08:/etc/profile done");
		 ...
		 */
		KTRACE_LOG("hush:09:sourcing user profiles");
		source_user_startup_files(user_profiles);
		KTRACE_LOG("hush:0A:user profiles done");
	}

	KTRACE_LOGF("%s=%d","hush:argv1", (int)(unsigned long)G.global_argv[1]);
	if (G.global_argv[1]) {
		FILE *input;
		/*
		...
		G.global_argc--;
		G.global_argv++;
		KTRACE_LOG("hush:script");
		if (G.global_argv[0]) KTRACE_LOG(G.global_argv[0]);
	 ...
	 */
#if ENABLE_HUSH_JOB
	KTRACE_LOGF("%s=%d","hush:0B:isatty(stdin)", isatty(STDIN_FILENO));
	KTRACE_LOGF("%s=%d","hush:0C:isatty(stdout)", isatty(STDOUT_FILENO));
...
# else
		if (G_saved_tty_pgrp) {
			 ...
			 * original code (but I think it actually doesn't). */
			tcnewpgrp(G_interactive_fd);
			install_fatal_sighandlers();
			install_special_sighandlers();
			settpgrp(G_interactive_fd);
...
# endif
	...
	if (G_interactive_fd && G.root_pid == getpid()) {
		KTRACE_LOG("hush:rc:sourcing");
		source_user_startup_files(user_rcs);
		KTRACE_LOG("hush:rc:done");
		install_special_sighandlers();
	}

	KTRACE_LOG("hush:banner");
	...
	KTRACE_LOG("hush:banner:done");

	KTRACE_LOG("hush:0D:entering stdin loop");
	parse_and_run_file(stdin);
	KTRACE_LOG("hush:0E:stdin loop returned");
...
# else
		...
		for (i = 0; i < pi->num_cmds; i++) {
			...
			if (pid != 0 && tctpgrp(G_interactive_fd, pid) != -1)
				break;
		}
# endif
	}
